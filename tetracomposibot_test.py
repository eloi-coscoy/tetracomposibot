...
def simulate(my_robots):
    ...
    while iteration < max_iterations:
        ...
        for k in range(len(my_robots)):
            ...
            resultat = my_robots[k].step(sensor_values, sensor_view, sensor_robot, sensor_team)

            # 2. On vérifie la taille du résultat
            if len(resultat) == 4:
                # Cas complet : le robot gère le changement d'arène
                translation_per_step, rotation_per_step, ask_for_reset, change_arena = resultat
            else:
                # Cas incomplet : on récupère les 3 valeurs et on force la 4ème à False
                translation_per_step, rotation_per_step, ask_for_reset = resultat
                change_arena = False
