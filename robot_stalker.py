...
class Robot_player(Robot):
    ...
    def step(self, sensors, sensor_view=None, sensor_robot=None, sensor_team=None):
        ...
        translation = sensor_to_robot[sensor_front]*0.98 + (1-sensor_to_robot[sensor_front])*0.97 + (1-sensor_to_robot[sensor_rear])*-0.96 + (1-sensor_to_wall[sensor_front])*-1 + (1-sensor_to_wall[sensor_front_left])*-0.3 + (1-sensor_to_wall[sensor_front_right])*-0.3 # A MODIFIER
        ...
        # on va faire un robot qui suit les autres robots, mais dans un premier temps il va chercher par defaut sasn coller au mur + par defaut rotation vers la droite car front est sur le diagonale de bas droite par défaut // après des test vaut mieux par mettre le cas de derrière moins éfficace (possible mais faudrait combiner avec l'option reculer, mais là, notre robot ne sait qu'avancer ce qui simplifie beaucoup) -> (1-sensor_to_robot[sensor_rear]) 
                    ...
                    # devant gauche,                                  devant droite,                                gauche                                 doite                                    arrière gauche                          arrière droite                                 mouvement débloquage lors détection mur (par defaut = gauche) valeur < 1 car avoir moins d'importance si un robot en vue (1 ou -1) 
        rotation =  (1-sensor_to_robot[sensor_front_left])*0.8 + (1-sensor_to_robot[sensor_front_right])*-0.8 + (1-sensor_to_robot[sensor_left]) + (1-sensor_to_robot[sensor_right])*-1 + (1-sensor_to_robot[sensor_rear_left]) + ((1-sensor_to_robot[sensor_front_right])*-1) + (1-sensor_to_wall[sensor_front])*0.7 + (1-sensor_to_wall[sensor_right])*0.5 + ((1-sensor_to_wall[sensor_left])*-1)*0.5
